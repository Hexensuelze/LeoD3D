;;; m32script-mode.el -- Major mode for editing Mapster .m32 files
...
;; Last updated by LeoD: 2025-06-27 (Mapster32 r10618 - r10618 -> r10619)
...
;;  (setq auto-mode-alist (cons '("\\.\\([Mm]32\\)$" . m32script-mode) auto-mode-alist))
...
;; static keywords
(defconst m32script-keywords-static
  '( \#define define definequote
 array gamearray gamevar var include includeoptional
 ))

;; block-enclosing keywords
(defconst m32script-keywords-block
  '( defstate ends onevent endevent ))

;; control flow keywords
(defconst m32script-keywords-flow
  '( break case default else endswitch for getcurraddress
 ifa ifae ifb ifbe ifl ifle ifg ifge ife ifn ifand ifor ifxor ifeither ifboth
 ifvarl ifvarle ifvarg ifvarge ifvara ifvarae ifvarb ifvarbe ifvare ifvarn
 ifvarand ifvaror ifvarxor
 ifvareither ifvarboth  ifvarvarl ifvarvarle ifvarvarg
 ifvarvarge ifvarvara ifvarvarae ifvarvarb ifvarvarbe ifvarvare ifvarvarn
 ifvarvarand ifvarvaror ifvarvarxor
 ifvarvareither ifvarvarboth
 ifhitkey ifhighlighted ifholdkey ifrnd ifangdiffl ifspritepal ifactor ifsound
 ifpdistl ifpdistg ifin3dmode ifin2d3dmode ifinside ifinteractive
...
(defconst m32script-keywords-4
  '( activelights allsectors allsprites
 allwalls drawnsprites lights loopofwall range
 selsec selsectors selspr selsprites selwalls
 spritesofsector sprofsec wallsofsector walofsec
;; ))
;; (defconst m32script-objects '( 
 sector sprite wall
 ))

;;(defconst m32script-keywords-4
(defconst m32script-keywords-5
  '( nullop state
 a2xy ah2xyz al shiftl shiftr rand set add sub mul div mod and or xor setsector
 getsector setwall getwall setsprite getsprite gettspr settspr setarray
 getarraysize resizearray copy randvar displayrandvar setvar addvar subvar
 mulvar divvar modvar andvar orvar xorvar shiftvarl shiftvarr shiftvarvarl
 shiftvarvarr randvarvar displayrandvarvar setvarvar addvarvar subvarvar
 mulvarvar divvarvar modvarvar andvarvar orvarvar xorvarvar sin cos displayrand
 itof ftoi clamp inv sqrt mulscale divscale scalevar dist ldist getangle
 getincangle globalsound getsoundflags calchypotenuse clipmove clipmovenoslide
 lineintersect rayintersect sort getspritelinktype  resetkey setkey
 insertsprite dupsprite tdupsprite deletesprite lastwall updatecursectnum
 updatesector updatesectorz updatesectorneighbor updatesectorneighborz
 getzrange hitscan cansee canseespr neartag
 rotatepoint dragpoint getceilzofslope getflorzofslope alignceilslope
 alignflorslope bsetsprite setfirstwall changespritestat changespritesect
 headspritestat prevspritestat nextspritestat headspritesect prevspritesect
 nextspritesect sectorofwall fixrepeats getclosestcol
 addlogvar addlog debug
 redefinequote print quote error printmessage16 printmessage256 printext256
 printext16 drawlabel getnumber16 getnumber256 getnumberfromuser qsprintf
 qstrcat qstrcpy qstrlen qstrncat qsubstr
 findnearsprite findnearspritevar findnearsprite3d
 findnearsprite3dvar findnearspritez findnearspritezvar
 getticks gettimedate setaspect seti sizeat cstat cstator clipdist
 spritepal cactor spgetlotag spgethitag sectgetlotag sectgethitag
 gettexturefloor gettextureceiling
 sethighlight sethighlightsector collectsectors sound soundonce stopallsounds
 stopsound updatehighlight updatehighlightsector
 drawline16 drawline16b drawline16z drawline256
 drawcircle16 drawcircle16b drawcircle16z
 rotatespritea rotatesprite16 rotatesprite setgamepalette
 xfliphighlightedsectors   yfliphighlightedsectors
 xmirrorhighlightedsectors ymirrorhighlightedsectors
 ))

(defconst m32script-sector-labels
  '( wallptr wallnum ceilingz
 ...
 floorypanning visibility fogpal lotag hitag extra
 alignto ceilingheinum floorheinum
 ))

(defconst m32script-wall-labels
  '( x y point2 nextwall nextsector cstat picnum overpicnum shade pal
 xrepeat yrepeat xpanning ypanning lotag hitag extra
 ))

(defconst m32script-sprite-labels
  '( x y z cstat picnum shade pal
 clipdist blend xrepeat yrepeat xoffset yoffset sectnum statnum
 ang owner xvel yvel zvel lotag hitag extra
;; detail aka filler
;; detail filler
 ))

(defconst m32script-light-labels
  '( x y z horiz range angle faderadius radius sector
 r g b priority tilenum minshade maxshade active
 ))
...
(defvar m32script-iter-keywords   'm32script-iter-keywords)
...
(defface m32script-block-keywords  '((t (:inherit font-lock-type-face)))
...
(defface m32script-iter-keywords   '((t (:inherit font-lock-variable-name-face)))
  "*Iteration list keywords for M32script Mode."  :group 'm32script :group 'faces)
...
(defface m32script-label-keywords  '((t (:inherit font-lock-function-name-face)))
;;(defface m32script-label-keywords  '((t ()))
  "*Sector/sprite/light/wall labels for M32script Mode." :group 'm32script :group 'faces)

(defconst m32script-font-lock-keywords-static
  `(
    (,(concat "\\_<" (regexp-opt 
                      (mapcar 'symbol-name m32script-keywords-static)
                      t)
...
(defconst m32script-font-lock-keywords-block
  (append m32script-font-lock-keywords-static
          `(
            (,(concat "\\b" (regexp-opt
                              (mapcar 'symbol-name m32script-keywords-block) t) "\\_>")
             . m32script-block-keywords) ) )
  "Block-enclosing primitives highlighted in M32SCRIPT mode.")

(defconst m32script-font-lock-keywords-flow
  (append m32script-font-lock-keywords-block
          `(
            (,(concat "\\_<" (regexp-opt
                              (mapcar 'symbol-name m32script-keywords-flow) t) "\\_>")
             . m32script-flow-keywords) ) )
  "Control-flow primitives highlighted in M32SCRIPT mode.")

(defconst m32script-font-lock-keywords-4
  (append m32script-font-lock-keywords-flow
          `(
            (,(concat "\\_<" (regexp-opt
                              (mapcar 'symbol-name m32script-keywords-4) t) "\\_>")
             . m32script-iter-keywords) )
          )
  "Iteration tokens highlighted in M32SCRIPT mode.")

(defconst m32script-font-lock-keywords-5
  (append m32script-font-lock-keywords-4
          `(
            (,(concat "\\_<" (regexp-opt
                              (mapcar 'symbol-name m32script-keywords-5) t) "\\_>")
             . m32script-misc-keywords) )
          )
  "All other primitives highlighted in M32SCRIPT mode.")

(defconst m32script-font-lock-keywords-6
  (append 
          `(
            (,(concat "\\.\\<" (regexp-opt
                              (append
                               ...
                               (mapcar 'symbol-name m32script-light-labels)
                               (mapcar 'symbol-name m32script-wall-labels)
                               )
                              t) "\\_>")
             . m32script-label-keywords) )
          m32script-font-lock-keywords-5 )
...
(defvar m32script-font-lock-keywords m32script-font-lock-keywords-6
...
(defun m32script-mode ()
  "Major mode for editing Mapster32 .m32 [.con] files"
  (interactive)
